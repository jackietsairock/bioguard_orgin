...
$rs = $mysqli->query("SELECT `id` FROM `news_update_english`");
...
$rs2 = $mysqli->prepare("SELECT * FROM `news_update_english` ORDER BY `id` DESC LIMIT ?, ? ");
...
<div class="container">
    ...
    <h1><b class="blue">百衛最新消息中(繁)</b></h1>
    </br>
    <a href="bioguard_edit_news_english.php"><p class="font_size21 red">→ 回到 新增最新消息 ←</p></a>
...
<script>
    function delete_it(id) {
        if(confirm('確定要刪除編號為 '+id+' 的資料嗎?')) {
            location.href = '__news_data_delete_english.php?id='+id;
        }
    }
</script>
</body>
