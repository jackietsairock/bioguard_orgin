...
$rs = $mysqli->query("SELECT `id` FROM `news_update_ch`");
...
$rs2 = $mysqli->prepare("SELECT * FROM `news_update_ch` ORDER BY `id` DESC LIMIT ?, ? ");
...
<div class="container">
    ...
    <a href="bioguard_edit_news_ch.php"><p class="font_size21 red">→ 回到 新增最新消息 ←</p></a>
...
<script>
    function delete_it(id) {
        if(confirm('確定要刪除編號為 '+id+' 的資料嗎?')) {
            location.href = '__news_data_delete_ch.php?id='+id;
        }
    }
</script>
</body>
